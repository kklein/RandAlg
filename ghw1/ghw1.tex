...
\begin{align*}
&\lim_{n \to \infty} (\frac{n-2}{n-1})^{c \cdot nlog(n)} \\
&= \lim_{m \to \infty} (\frac{m}{m+1})^{c \cdot (m+2)log(m+2)} \\ 
&=  \lim_{m \to \infty} ((\frac{1}{1 + \frac{1}{m}})^m(\frac{1}{1 + \frac{1}{m}})^2)^{log(m+2)} \\
&= \lim_{m \to \infty} (\frac{1}{e} \cdot 1)^{log(m+2)} \label{eq3}\\
& \to 0
\end{align*}
\begin{align*}
...
&= 1 - (\frac{n-2}{n-1})^k \\
&\to 1 \text{ (with large enough } n, k = c \cdot nlog(n) \text{ and } \ref{eq3} \text{)}
\end{align*}
We will now show that each position is indepdendent of each other.


\begin{align*}
&\Pr[\text{for each position } p \text{ each } c_i \neq a_p \text{ has been drawn among } k \text{ zero-guesses, drawn u.a.r.}] \\
&= \prod_{p = 1}^n \Pr[X_p] \\
&= (1 - (\frac{n-2}{n-1})^k)^n \text{ (via independence of positions)} \\
&\to 1 \text{ (via the previous limit)}
\end{align*}

Summing up, we have established that we can expect \(c'\cdot log(n) \) many zero-guesses from \(c'\cdot e \cdot log(n)\) total guesses, which are sufficient to determine to codeword whp.
